...
public class Route implements Iterable<Stop> {
    ...
    private List<RoutePattern> routePatternList;
    ...
    public Route(String number) {
        ...
        this.routePatternList = new ArrayList<>();
    ...
    public void addPattern(RoutePattern pattern) {
        if (!this.routePatternList.contains(pattern)) {
            routePatternList.add(pattern);
        }
    }
    ...
    public RoutePattern getPattern(String patternName, String destination, String direction) {
        ...
        for (int i = 0; i < routePatternList.size(); i++) {
            if (routePatternList.get(i).getName().equals(patternName)) {
                routePattern = routePatternList.get(i);
            }
        }
        if (routePattern == null) {
            routePattern = new RoutePattern(patternName, destination, direction, this);
    ...
    public RoutePattern getPattern(String patternName) {
        ...
        if (routePattern == null) {
            routePattern = new RoutePattern(patternName, "", "", this);
            addPattern(routePattern);
        }
        return routePattern;
    }
    ...
    public List<RoutePattern> getPatterns() {
        return Collections.unmodifiableList(this.routePatternList);
    }
}
