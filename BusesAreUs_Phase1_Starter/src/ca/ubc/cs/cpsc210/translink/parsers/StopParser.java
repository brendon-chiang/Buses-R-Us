...
public class StopParser {
    ...
    public void parseStops(String jsonResponse)
        ...
        for (int i = 0; i < stops.length(); i++) {
            ...
            if (!eachStop.has("Name") || !eachStop.has("StopNo") || !eachStop.has("Routes") || !eachStop.has("Latitude") || !eachStop.has("Longitude")) {
                throwException = true;
            }
            else {
                stopManager.getStopWithNumber(eachStop.getInt("StopNo"), eachStop.getString("Name"), new LatLon(eachStop.getDouble("Latitude"), eachStop.getDouble("Longitude")));
            }
        }
        if (throwException)
